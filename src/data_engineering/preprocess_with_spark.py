...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description="Spark Preprocessing Script for Fraud Detection Data")
    parser.add_argument("--input_file", type=str, required=True, help="Path to the input raw CSV file (e.g., data/raw/full_fraud_data.csv).")
    parser.add_argument("--output_suffix", type=str, required=True, help="Suffix for the output directory (e.g., 'full' or 'sample_1M').")
    return parser

def main(args):
    """Main function to run the Spark preprocessing job."""
    logging.info(f"Starting Spark preprocessing job for data version: {args.output_suffix}")
    ...
    try:
        ...
        spark = SparkSession.builder \
            .appName(f"FraudDataPreprocessing_{args.output_suffix}") \
            .master("local[*]") \
            .getOrCreate()
        ...
        logging.info(f"Raw data loaded from {raw_data_path}. Row count: {raw_df.count()}")
        ...
        types = [row['type'] for row in processed_df.select('type').distinct().collect()]
        for t in types:
            processed_df = processed_df.withColumn(f"type_{t}", when(col("type") == t, 1).otherwise(0))
        ...
        logging.info(f"Processed data saved to {processed_data_path} in Parquet format.")
