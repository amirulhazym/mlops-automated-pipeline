...
from sklearn.preprocessing import StandardScaler # <-- ADDED for scaling data for the neural network
...
def setup_arg_parser():
    ...
    parser = argparse.ArgumentParser(description='Model Training Script for Fraud Detection')
    
    # --- General arguments ---
    parser.add_argument(
        "--data_version", type=str, default="sample_1000000",
        help="Version of the data to use (e.g., 'sample_1000000' or 'full')."
    )
    parser.add_argument("--run_name", type=str, required=True, help="Name for the MLflow run.")
    ...
    parser.add_argument("--learning_rate", type=float, default=0.001, help="Learning rate for the optimizer (used by both models).")
    parser.add_argument("--epochs", type=int, default=10, help="Number of epochs for TensorFlow model.")
    parser.add_argument("--batch_size", type=int, default=256, help="Batch size for TensorFlow model.")
...
def train_tensorflow_mlp(X_train, y_train, X_test, y_test, args):
    ...
    model = tf.keras.Sequential([
        ...
        tf.keras.layers.Dropout(0.3),
        tf.keras.layers.Dense(1, activation='sigmoid') # Sigmoid for binary probability output
    ])
    ...
    final_metrics = {"accuracy": accuracy_score(y_test, preds), "f1_score": f1_score(y_test, preds), "roc_auc": roc_auc_score(y_test, pred_proba)}
...
def train_xgboost(X_train, y_train, X_test, y_test, args):
    ...
    metrics = {"accuracy": accuracy_score(y_test, preds), "f1_score": f1_score(y_test, preds), "roc_auc": roc_auc_score(y_test, pred_proba)}
...
def main(args):
    ...
    with mlflow.start_run(run_name=args.run_name):
        logging.info(f"Starting MLflow run: '{args.run_name}' for model type: '{args.model_type}'")
